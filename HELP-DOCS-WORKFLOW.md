...
- **`scripts/format-data.mjs`** — 定义 27 种文件格式的元数据（扩展名、分组、渲染方式、中英文描述）。`FORMAT_GROUPS` 和 `RENDER_HINT_LABELS` 已扩展到全部 20 种语言（未翻译的语种回退到英文）。
- **`scripts/generate-format-pages.mjs`** — 遍历全部 20 种语言 × 27 种格式，为每种组合生成单独的 `.md` 页面。格式描述在非中文/非英文语种中使用英文回退。
...
每种语言下自动生成 `formats/index.md`，列出所有 27 种格式的链接。索引页面由 `generate-format-pages.mjs` 自动生成。
