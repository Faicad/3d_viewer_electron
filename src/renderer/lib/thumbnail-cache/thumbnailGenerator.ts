...
/**
 * Generate a placeholder thumbnail for DWG files (Phase 1).
 *
 * Phase 1 uses a simple Canvas 2D placeholder showing the DWG brand
 * text and file name, avoiding GPL-3.0 WASM loading for thumbnail
 * generation. Phase 2 will replace this with real embedded thumbnail
 * extraction via dwg_bmp() and/or SVG-rendered thumbnails.
 */
export async function generateDwgPlaceholder(
  fileName: string,
): Promise<Blob | null> {
  const W = 200, H = 150
  const canvas = document.createElement('canvas')
  ...
  // Background: light matte + DWG brand border
  ...
  // Centered extension brand text
  ctx.fillStyle = '#f97316'
  ctx.font = 'bold 36px sans-serif'
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  ctx.fillText('DWG', W / 2, H / 2 - 10)
  ...
  if (ctx.measureText(displayName).width > maxW) {
    while (ctx.measureText(displayName + '…').width > maxW && displayName.length > 2)
      displayName = displayName.slice(0, -1)
    displayName += '…'
  }
  ctx.fillText(displayName, W / 2, H / 2 + 28)
