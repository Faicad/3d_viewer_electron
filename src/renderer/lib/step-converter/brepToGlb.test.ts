 ...
 *
 * Integration test: BREP → GLB conversion using the production pipeline.
 *
 * BREP follows the same CAD→GLB path as STEP: occt-import-js ReadBrepFile
 * converts BREP geometry natively. Tests that the GLB is structurally valid.
 */
...
beforeAll(async () => {
  ...
  const brepPath = join(PROJECT_ROOT, 'src', 'test', 'fixtures', 'Motor-c.brep')
  const brepBuf = readFileSync(brepPath)
  const brepData = new Uint8Array(
    brepBuf.buffer,
    brepBuf.byteOffset,
    brepBuf.byteLength,
  )

  const result = occtModule.ReadBrepFile(brepData, null)
...
describe('BREP → GLB production pipeline', () => {
