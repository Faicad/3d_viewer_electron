export function startAssembleDemo(): () => void {
  ...
  #${panelId} .ctrl-row .value { font-size: 11px; color: #44aaff; font-weight: 600; min-width: 24px; text-align: right; font-variant-numeric: tabular-nums; }
  ...
  #${panelId} input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; width: 10px; height: 10px; border-radius: 50%; background: #44aaff; cursor: pointer; border: 2px solid #0d0d1a; transition: transform 0.1s; }
  #${panelId} input[type="range"]::-webkit-slider-thumb:hover { transform: scale(1.2); }
  #${panelId} input[type="range"]::-moz-range-thumb { width: 10px; height: 10px; border-radius: 50%; background: #44aaff; cursor: pointer; border: 2px solid #0d0d1a; }
  ...
  function captureParts() {
    ...
    if (!partInfos || !partInfos.length) return false
    ...
    for (let i = 0; i < partInfos.length; i++) {
      ...
      parts.push({ partId: info.partId, proxy, localPos, worldPos, height, name: info.name })
    }

    if (!parts.length) return false

    const box = new THREE.Box3()
    for (let j = 0; j < worldPositions.length; j++) box.expandByPoint(worldPositions[j])
    const assemblyTopZ = box.max.z
    _assemblyData = { parts, assemblyTopZ }
    return true
  }
