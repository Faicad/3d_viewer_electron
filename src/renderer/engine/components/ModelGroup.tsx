...
import { getDefaultUpAxis, isStepFile, isIgesFile, isBrepFile } from '@/config/file-formats'
...
const ModelGroup = forwardRef<THREE.Group, ModelGroupProps>(function ModelGroup(
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // STEP/IGES/BREP are special — should have been converted to GLB already
        if (isStepFile(format) || isIgesFile(format) || isBrepFile(format)) {
          console.warn('[ModelGroup] CAD format received without prior conversion -- should be GLB by now')
          return
        }
        ...
        if (result.objects.length > 0 && result.meshes.length === 0) {
          ...
          // G-code: skip any vertex where XY ≈ (0,0).  The file always starts at
          // the origin and the first few moves (Z lift, priming extrusion) stay at
          // XY=(0,0).  These origin-anchored vertices would pull the bounding-box
          // center toward the bottom-left corner instead of the actual toolpath.
            ...
            if (format === 'gcode' && obj instanceof THREE.LineSegments) {
              ...
              if (pos) {
                ...
                for (let i = 0; i < pos.count; i++) {
                  ...
                  if (Math.abs(vec.x) < EPS && Math.abs(vec.y) < EPS) continue
