...
export async function exportTo3MF(
  ...
  const level = printConfig.compression === 'standard' ? 6 : 0
  ...
  const u8 = zipSync({
    ...
    '3D/3dmodel.model': strToU8(mainModelXml),
    'Metadata/model_settings.config': strToU8(modelSettingsXml),
    'Metadata/project_settings.config': strToU8(projectSettingsConfig),
    '[Content_Types].xml': strToU8(contentTypesXML()),
  }, { level })
  return new Blob([u8], { type: 'application/vnd.ms-package.3dmanufacturing-3dmodel+xml' })
}
