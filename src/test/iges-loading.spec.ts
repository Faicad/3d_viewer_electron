/**
 * E2E test: IGES file loading via file input.
 *
 * IGES shares the same occt-import-js WASM conversion pipeline as STEP.
 * Validates the full asynchronous flow: file input → OCCT ReadIgesFile → GLB → mesh render.
 */
...
const IGES_FIXTURE = readFileSync(path.join(__dirname, 'fixtures', 'hammer.iges'))
...
test.describe('3D Viewer Electron - IGES Loading', () => {
  ...
  test('loads IGES file, converts to GLB, renders mesh with topology', async () => {
    const window = await electronApp.firstWindow()
    ...
    expect(file.fileName).toContain('.iges')
    expect(file.format).toBe('glb')
    expect(file.glbPartInfos.length).toBeGreaterThan(0)

    console.log('[test] IGES file loaded:', JSON.stringify({
  ...
  test('loads IGES via loadModel command and validates model info', async () => {
    const window = await electronApp.firstWindow()
    test.skip(isSoftwareGpu(), 'WebGL2 thumbnail generation fails on CI / software GPU')
    const { assertNoErrors } = trackErrors(window)

    const IGES_DATA_URL = `data:application/iges;base64,${IGES_FIXTURE.toString('base64')}`

    const result = await window.evaluate(async (dataUrl) => {
      const resp = await window.__executeCommand('loadModel', { data: dataUrl, fileName: 'hammer.iges' })
    ...
    expect(result.info.data.fileName).toContain('.iges')

    console.log('[test] IGES loadModel result:', JSON.stringify({
